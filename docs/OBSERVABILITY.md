...
NovaSight emits **Prometheus metrics** and **OpenTelemetry traces** across the API,
...
| Signal | Where | Source |
|--------|-------|--------|
| **Request** metrics | API `:8000/metrics` | [`PrometheusHTTPMiddleware`](../backend/app/core/metrics.py) — `novasight_http_requests_total`, `novasight_http_request_duration_seconds` (labeled by method, route template, status) |
| **Pipeline** metrics | wherever ingestion runs, on `/metrics` | [`csv_iceberg`](../backend/app/ingestion/csv_iceberg.py) — `novasight_ingest_rows_total`, `novasight_ingest_duration_seconds` |
...
| `OBSERVABILITY__SERVICE_NAME` | `novasight` | base `service.name` |
...
- **Grafana** http://localhost:3001 — auto-provisioned Prometheus + Jaeger datasources
  and the *NovaSight — Overview* dashboard (request rate, p95 latency, ingest rows,
  worker jobs, API memory).
- **Prometheus** http://localhost:9090 — targets `novasight-api` and `novasight-worker`.
- **Jaeger** http://localhost:16686 — pick service `novasight-api` to see a request's
