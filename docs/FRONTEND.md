...
| `/build` | Chart builder | **Three-pane** low-code builder over **semantic models**: a left **Fields** rail (draggable governed dimensions/measures), a center workspace with the **shelf-pills bar** (**X-axis** / **Breakdown** / **Metrics** / **Filters**) above a **hero chart preview** + an inline **NL→chart** bar, and a right rail for **chart type** + **formatting** + **query options** (granularity for a `time` X, sort, row limit). Multiple measures + breakdown dimensions pivot into series (`chartPivot`); **Saved charts** open in a drawer; **Save chart** (server-side, `POST /charts`) or add to dashboard. One shared `DndContext` (lifted in `SemanticQueryBuilder` → `BuilderDnd`) spans the panes so fields drag onto shelves across columns |
| `/dashboards` | Dashboards | List / create dashboards |
| `/dashboards/:dashboardId` | Dashboard detail | **Command-center** layout: native filters in a **horizontal top filter-bar** (`DashboardFilterBar`) over a **full-width gridstack canvas** of tiles (`DashboardCardTile`, glass chrome + hover-lift); view-time filtering, cross-filter (click a point), drill, and edit mode (gridstack reorder/resize/remove) |
